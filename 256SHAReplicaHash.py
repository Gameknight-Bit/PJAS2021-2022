...
def binaryToHex(b):
    dec = int(b, 2)
    hexThing = str(hex(dec))[2:]
    while len(hexThing)%8 != 0: #Padding
        hexThing = "0"+hexThing
    return hexThing
...
def hash(k): #Main Hashing Algorithm
    ...
    #SPLIT BINARY INTO 512 bit sections!!!!!#
    chunks = []
    for o in range(math.floor(len(binaryKey)/512)):
        chunks.append(binaryKey[:512])
        if len(binaryKey) <= 512:
            binaryKey = ""
        else:
            binaryKey = binaryKey[512:]
    ...
    for chunk in chunks:
        ...
        while scheduleLen < 16: #Word Processing (32-bit sections)
            mSchedule.append(chunk[scheduleLen*32:(scheduleLen+1)*32])
            scheduleLen += 1
